...
class VLMEnvMixin:
    def __init__(self, max_steps: int = 5, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.env_feedback = None
        self.max_steps = max_steps

    def get_prompt(self, **kwargs) -> str:
        """Get the prompt for the environment i.e., instructions for the VLM to complete the task."""
        raise NotImplementedError

    def solve(self, **kwargs):
        """Solve the environment and return the solution trajectory."""
        raise NotImplementedError
...
class Wrapper(
    ...
    def get_prompt(self, **kwargs) -> str:
        """Get the prompt for the environment from the wrapped env (VLMEnvMixin).
        
        This forwards to the wrapped environment's get_prompt method if it exists.
        """
        return self.env.get_prompt(**kwargs)

    def solve(self, **kwargs):
        """Solve the environment and return the solution trajectory.
        
        This forwards to the wrapped environment's solve method if it exists.
        """
        return self.env.solve(**kwargs)
