...
class HopperVLMEnv(HopperEnv, gym.VLMEnvMixin):
    def __init__(self, **kwargs):
        # 1) init the underlying Mujoco-hop­per
        super().__init__(**kwargs)
        # 2) switch to image‐mode
        self.render_mode = "rgb_array"
        # 3) one reset+render just to bind shapes
        super().reset()
        frame = super().render()
        H, W, C = frame.shape
        # 4) now override spaces to match “image in, text‐action out”
        self.observation_space = Box(0, 255, shape=(H, W, C), dtype=np.uint8)
        self.action_space = FuncConditional({
            "act":  Box(low=-1.0, high=1.0, shape=(3,), dtype=np.float32),
            "stop": Text(4)
        })
        # keep the last rendered frame
        self._frame = frame

    
    def get_prompt(self, **kwargs) -> str:
        return (
            "You are controlling a one-legged Hopper robot.  The observation is an RGB image showing:\n"
            "  • the torso, thigh, leg, and foot segments\n"
            "  • the ground and surrounding environment\n\n"
            "Your objective is to make the Hopper move forward (to the right) by applying torques at its three hinge joints.\n\n"
            "At each step, output exactly one action as a string, in one of these two formats:\n\n"
            "  1. Operate:\n"
            "     (\"act\", [τ_thigh, τ_leg, τ_foot])\n"
            "     • τ_thigh ∈ [−1, 1]: torque on the thigh joint\n"
            "     • τ_leg   ∈ [−1, 1]: torque on the leg joint\n"
            "     • τ_foot  ∈ [−1, 1]: torque on the foot joint\n\n"
            "  2. Submit and end the episode:\n"
            "     (\"stop\", \"stop\")\n\n"
            "Examples:\n"
            "  (\"act\", [0.2, -0.5, 0.1])   \n"
            "  (\"act\", [-1.0, 0.0, 1.0])   \n"
            "  (\"stop\", \"stop\")          # end the episode now\n\n"
            "Respond with exactly one of these tuples—nothing else."
        )


    def step(self, action_str: str):
        # parse the VLM’s command
        func, args = ast.literal_eval(action_str)
        if func == "stop":
            # simulate one zero‐torque step so we get a final reward/info
            zero = np.zeros(3, dtype=np.float32)
            _, reward, done, truncated, info = super().step(zero)
            # force termination immediately
            done = True
            # render that final frame
            frame = super().render()
            return frame, reward, done, truncated, info

        if func != "act":
            raise ValueError(f"Unknown action {func!r}, expected 'act' or 'stop'")

        # unpack and apply the three torques
        if not (isinstance(args, (list, tuple)) and len(args) == 3):
            raise ValueError("‘act’ requires a list of three numbers")
        a, b, c = float(args[0]), float(args[1]), float(args[2])
        obs, reward, done, truncated, info = super().step(np.array([a, b, c], dtype=np.float32))
        frame = super().render()
        return frame, reward, done, truncated, info