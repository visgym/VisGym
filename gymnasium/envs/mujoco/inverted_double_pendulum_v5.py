...
class InvertedDoublePendulumVLMEnv(InvertedDoublePendulumEnv, gym.VLMEnvMixin):
    def __init__(self, **kwargs):
        # 1) init the underlying MuJoCo env
        super().__init__(**kwargs)
        # 2) switch to image mode
        self.render_mode = "rgb_array"
        # 3) burn one reset+render to fix frame size
        super().reset()
        frame = super().render()
        H, W, C = frame.shape

        # 4) override spaces to match “image in, text out” API
        self.observation_space = Box(
            low=0, high=255, shape=(H, W, C), dtype=np.uint8
        )
        self.action_space = FuncConditional({
            "act":  Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32),
            "stop": Text(4),
        })

    def get_prompt(self, **kwargs) -> str:
        return (
            "You control an inverted double pendulum mounted on a cart.  The observation is an RGB image showing:\n"
            "  • the cart on its track\n"
            "  • a lower pole attached to the cart\n"
            "  • an upper pole attached to the end of the lower pole\n\n"
            "Your objective is to keep both poles balanced upright by pushing the cart left or right.\n\n"
            "At each step, output exactly one action as a string, in one of these two formats:\n\n"
            "  1. Operate:\n"
            "     (\"act\", [f])\n"
            "     • f: horizontal force on the cart ∈ [−1, 1]\n\n"
            "  2. Submit and end the episode:\n"
            "     (\"stop\", \"stop\")\n\n"
            "Examples:\n"
            "  (\"act\", [0.5])    # push the cart right with force 0.5\n"
            "  (\"act\", [-0.3])   # push the cart left with force 0.3\n"
            "  (\"stop\", \"stop\")\n\n"
    ...
    def step(self, action_str: str):
        # 1) parse the VLM’s string
        func, args = ast.literal_eval(action_str)

        if func == "stop":
            # run one zero-action step to collect final reward & info
        ...
        # 2) extract the single force
        if not (isinstance(args, (list, tuple)) and len(args) == 1):
            raise ValueError("`act` requires a list of one number: ['act', [f]]")
        f = float(args[0])
