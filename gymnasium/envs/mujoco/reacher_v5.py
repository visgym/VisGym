...
class ReacherVLMEnv(ReacherEnv, gym.VLMEnvMixin):
    def __init__(self, *, seed: int = None, **kwargs):
        # initialize the MuJoCo env (which itself does EzPickle etc)
        super().__init__(**kwargs)
        # immediately switch to rgb_array mode
        self.render_mode = "rgb_array"
        
        self.reset(seed=seed)

        # run one reset+render just to get frame dims
        frame = super().render()
        H, W, C = frame.shape

        # override spaces to match our "text+image" API
        self.observation_space = Box(0, 255, shape=(H, W, C), dtype=np.uint8)
        self.action_space = FuncConditional({
            "act": Box(low=-1.0, high=1.0, shape=(2,), dtype=np.float32),
            "stop": Text(4)
        })
    ...
    def get_prompt(self, **kwargs) -> str:
        return (
            "You are controlling a 2-joint robotic arm.  The observation is an RGB image showing:\n"
            "  • the arm (two links connected by hinge joints)\n"
            "  • a target marker somewhere in view\n\n"
            "Your goal is to move the fingertip of the arm as close as possible to the target.\n\n"
            "At each step, output exactly **one** action as a string, in one of these two formats:\n\n"
            "  1. Operate:\n"
            "     (\"act\", [τ₁, τ₂])\n"
            "     • τ₁ is the torque on joint 0 (range −1 to 1)\n"
            "     • τ₂ is the torque on joint 1 (range −1 to 1)\n\n"
    ...
    def step(self, action_str: str):
        # parse your VLM’s string command
        func, args = ast.literal_eval(action_str)
        if func == "stop":
            # compute only the distance‐based reward (zero action → zero ctrl cost)
            # _get_rew expects an array-like action; with [0,0], reward_ctrl=0.
            obs, reward, done, truncated, info = super().step(np.array([0.0, 0.0], dtype=np.float32))
            done = True
            return obs, reward, done, truncated, info
        if func != "act":
            raise ValueError(f"Unknown action {func!r}")
        a, b = float(args[0]), float(args[1])
        # step the original ReacherEnv
        obs, reward, done, truncated, info = super().step(np.array([a, b], dtype=np.float32))
        return obs, reward, done, truncated, info
