...
class SwimmerVLMEnv(SwimmerEnv, gym.VLMEnvMixin):
    def __init__(self, **kwargs):
        # 1) init the base MuJoCo swimmer
        super().__init__(**kwargs)
        # 2) force RGB‐array rendering
        ...
        # 4) expose that as our obs and text+image API
    ...
    def get_prompt(self, **kwargs) -> str:
        return (
            "You are controlling a 2-link swimmer suspended in a 2D fluid.  The observation is an RGB image showing:\n"
            "  • the swimmer’s three segments and two rotor joints\n"
            "  • its orientation and surrounding fluid\n\n"
            "Your objective is to propel the swimmer as far to the right as possible by applying torques.\n\n"
            "At each step, output exactly one action as a string, in one of these two formats:\n\n"
            "  1. Operate:\n"
            "     (\"act\", [τ₁, τ₂])\n"
            "     • τ₁: torque at first rotor ∈ [−1, 1]\n"
            "     • τ₂: torque at second rotor ∈ [−1, 1]\n\n"
    ...
    def step(self, action_str: str):
        ...
        if func == "stop":
            # run one zero‐torque step to compute final reward
        ...
        # extract and validate torques
        if not (isinstance(args, (list,tuple)) and len(args) == 2):
            raise ValueError("`act` requires a two‐element list: ('act',[τ₁,τ₂])")
        τ1, τ2 = float(args[0]), float(args[1])

        # step the swimmer
        _, reward, done, truncated, info = super().step(np.array([τ1, τ2], dtype=np.float32))
        frame = super().render()
        return frame, reward, done, truncated, info
