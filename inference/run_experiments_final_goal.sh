...
OUTROOT="${SCRIPT_DIR}/eval_final_goal"
...
# Environments to run (can override via environment: ENVS="matchstick_equation")
: "${ENVS:=zoom_in_puzzle patch_reassembly colorization matchstick_equation}"

# Additional flags
MAKE_GIF="--make-gif"
EXTRA_FLAGS="--include-env-feedback"
PROVIDE_FINAL_GOAL="--provide-final-goal"
...
run_experiment() {
  ...
  # Set paths based on split and env
  local episodes_root="${DATASET_ROOT}/test_set_${split}/${env}__${split}"
  local initdir="${DATASET_ROOT}/initial_states_${split}"
  local outdir="${OUTROOT}/${model_tag}_${split}_final_goal/${env}"
  ...
  if [[ "$split" == "hard" ]]; then
    max_steps_flag=(--max-steps 30)
  elif [[ "$split" == "easy" ]]; then
    max_steps_flag=(--max-steps 20)
  fi


  echo "========================================"
  echo "Running: ${model_tag} on ${split} - ${env}"
...
for model_tag in ${MODELS}; do
  for split in ${SPLITS}; do
    for env in ${ENVS}; do
      run_experiment "$model_tag" "$split" "$env"
    done
  done
done
